...
export const Validators = {
  ...
  minLength: (length: number, message = `Minimum length is ${length} characters`): ValidationRule => ({
    validate: (value: any): boolean => typeof value === "string" && value.trim().length >= length,
    message,
  }),
  ...
  pattern: (pattern: RegExp, message = "Invalid format"): ValidationRule => ({
    validate: (value: any): boolean => typeof value === "string" && pattern.test(value),
    message,
  }),
  ...
  numeric: (message = "Please enter a number"): ValidationRule => ({
    validate: (value: any): boolean => !isNaN(parseFloat(value)) && isFinite(value),
    message,
  }),
  
  integer: (message = "Please enter a whole number"): ValidationRule => ({
    validate: (value: any): boolean => Number.isInteger(Number(value)),
    message,
  }),
  
  min: (min: number, message = `Minimum value is ${min}`): ValidationRule => ({
    validate: (value: any): boolean => Number(value) >= min,
    message,
  }),
  
  max: (max: number, message = `Maximum value is ${max}`): ValidationRule => ({
    validate: (value: any): boolean => Number(value) <= max,
    message,
  }),
